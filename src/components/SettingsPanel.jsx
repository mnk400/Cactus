...
function SettingsPanel({ 
  ...
  const photoPercentage = totalFiles > 0 ? Math.round((totalPhotos / totalFiles) * 100) : 0
  const videoPercentage = totalFiles > 0 ? Math.round((totalVideos / totalFiles) * 100) : 0

  const getButtonClass = (mediaType) => {
    const baseClass = "media-type-btn flex-1 border-none py-2 px-3 rounded-xl cursor-pointer text-sm font-medium transition-all duration-200 ease-in-out active:scale-95"
