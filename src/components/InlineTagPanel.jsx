...
const InlineTagPanel = memo(function InlineTagPanel({
  ...
  // Collapse panel when media changes
  useEffect(() => {
  ...
  // Collapsed: just show TagDisplay
  if (!isExpanded) {
    return (
      <TagDisplay
        tags={mediaTags}
  ...
  return (
    <div
      ref={panelRef}
      className="pointer-events-auto w-full overflow-hidden transition-all duration-300 ease-in-out mb-2"
    >
      <div className="bg-black-shades-800 backdrop-blur-md rounded-2xl p-3 space-y-3">
        {/* Row 1: All current tags expanded */}
        <TagDisplay
          tags={mediaTags}
          onRemoveTag={handleRemoveTag}
          expanded={true}
        ...
        {/* Row 2: Tag input + sparkle button */}
        <div className="flex items-start gap-2">
          ...
          {currentMediaFile && (
            <button
              ...
              className={`flex-shrink-0 bg-black-shades-700 text-gray-200 border-none px-3 py-2 rounded-lg cursor-pointer transition-all duration-200 ease-in-out hover:bg-white active:scale-95 flex items-center gap-2 ${
        ...
        {/* Row 3: AI suggestion chips (conditional) */}
        {aiState === "confirm" && (
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-sm font-medium transition-all duration-150 ${
                      isSelected
                        ...
                        : "bg-black-shades-700 text-zinc-400 line-through"
                    }`}
                  >
                    {tag}
                  </button>
