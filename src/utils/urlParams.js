...
export const encodeSettingsToURL = (settings) => {
  ...
  if (settings.selectedTags && settings.selectedTags.length > 0) {
    const tagNames = settings.selectedTags.map(tag => tag.name || tag).join(',');
    params.set(URL_PARAMS.SELECTED_TAGS, tagNames);
  }
  
  if (settings.excludedTags && settings.excludedTags.length > 0) {
    const tagNames = settings.excludedTags.map(tag => tag.name || tag).join(',');
 ...
 */
export const decodeSettingsFromURL = (searchParams = window.location.search) => {
  ...
  if (selectedTagsParam) {
    settings.selectedTags = selectedTagsParam.split(',').filter(tag => tag.trim());
  }
  
  // Parse excluded tags
  const excludedTagsParam = params.get(URL_PARAMS.EXCLUDED_TAGS);
  if (excludedTagsParam) {
    settings.excludedTags = excludedTagsParam.split(',').filter(tag => tag.trim());
  }
