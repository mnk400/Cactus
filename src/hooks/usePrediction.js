...
export const usePrediction = (predictApiUrl, onTagsPredicted) => {
  ...
  const predictTags = async (currentMediaFile) => {
    if (!currentMediaFile || (!isImage(currentMediaFile) && !isVideo(currentMediaFile)) || !predictApiUrl) {
      return;
    }
      ...
      if (isImage(currentMediaFile)) {
        ...
        const captureFrame = (videoElement, time) => {
            return new Promise((resolve, reject) => {
                ...
                videoElement.onerror = (err) => reject(new Error('Failed during frame capture.'));
            });
        };
        ...
        await new Promise((resolve, reject) => {
            video.onloadeddata = async () => {
                try {
                    ...
                    const [predictResponse1, predictResponse2, predictResponse3] = await Promise.all([
                    ...
                    if (!predictResponse1.ok || !predictResponse2.ok || !predictResponse3.ok) {
                    ...
                    const tags1 = data1.tags ? data1.tags.split(',').map(tag => tag.trim()).filter(tag => tag) : [];
                    const tags2 = data2.tags ? data2.tags.split(',').map(tag => tag.trim()).filter(tag => tag) : [];
                    const tags3 = data3.tags ? data3.tags.split(',').map(tag => tag.trim()).filter(tag => tag) : [];
