...
const useTags = () => {
    ...
    const getMediaTags = useCallback(async (filePath) => {
        setError(null);
        try {
            const response = await fetch(`/api/media-path/tags?path=${encodeURIComponent(filePath)}`);
            if (!response.ok) {
    ...
    const addTagsToMedia = useCallback(async (filePath, tagNames) => {
        setError(null);
        try {
            const response = await fetch(`/api/media-path/tags?path=${encodeURIComponent(filePath)}`, {
                method: 'POST',
    ...
    const removeTagFromMedia = useCallback(async (fileHash, tagId) => {
        setError(null);
        try {
