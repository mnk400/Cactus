...
export const MediaProvider = ({ children }) => {
    ...
    const updateTag = useCallback(async (id, name, color) => {
        try {
            ...
            const data = await response.json();
            setTags(prev => prev.map(tag => tag.id === id ? { ...tag, ...data.tag } : tag));
            return data.tag;
        } catch (err) {
    ...
    const fetchMedia = useCallback(async (overrides = {}) => {
        const params = {
            ...
            tags: (overrides.selectedTags || selectedTags).map(t => t.name || t).join(','),
            'exclude-tags': (overrides.excludedTags || excludedTags).map(t => t.name || t).join(','),
            pathSubstring: overrides.pathFilter !== undefined ? overrides.pathFilter : pathFilter,
        };

        const targetMediaId = overrides.mediaId || mediaId;
        ...
        try {
            ...
            if (params['exclude-tags']) url += `&exclude-tags=${params['exclude-tags']}`;
            if (params.pathSubstring) url += `&pathSubstring=${encodeURIComponent(params.pathSubstring)}`;
            ...
            if (targetMediaId && files.length > 0) {
                const foundIndex = files.findIndex(f => f.file_hash === targetMediaId);
                if (foundIndex !== -1) {
    ...
    }, [mediaType, sortBy, selectedTags, excludedTags, pathFilter, mediaId]);
    ...
    useEffect(() => {
        if (isUrlInitialized && tags.length >= 0) {
            fetchMedia();
        }
    }, [isUrlInitialized, tags.length, fetchMedia]);
    ...
    }, [currentIndex, currentMediaFile, isUrlInitialized, mediaId, isUrlSync, updateSetting]);
    ...
    const navigate = useCallback((direction) => {
        ...
        setCurrentIndex(prev => (prev + direction + mediaFiles.length) % mediaFiles.length);
    }, [mediaFiles.length]);

    // Public API
    const setFilters = useCallback((newSettings) => {
    ...
    const value = {
        ...
        canRegenerateThumbnails: config?.provider?.capabilities?.canRegenerateThumbnails || false,
